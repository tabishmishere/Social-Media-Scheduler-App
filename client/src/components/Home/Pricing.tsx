            ...
            <div className="max-w-6xl mx-auto px-4 sm:px-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-5 items-start">
                    {pricingPlans.map((plan) => (
                        <div key={plan.name} className={`card-hover rounded-2xl border p-7 flex flex-col gap-6 relative transition-all duration-300 ${plan.highlight ? "bg-red-500 text-white border-red-400 shadow-2xl shadow-red-500/20 scale-105" : "bg-white text-slate-900 border-slate-200"}`}>
                            ...
                            <Link to="/login" className={`btn-animate mt-auto text-center font-semibold text-sm px-6 py-3 rounded-full active:scale-95 ${plan.highlight ? "bg-white text-red-500 hover:bg-red-50 shadow-md" : "bg-red-500 text-white hover:bg-red-600 shadow-md"}`}>
                                {plan.cta}
                            </Link>
                        </div>
                    ))}
