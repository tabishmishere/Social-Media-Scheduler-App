...
import { ArrowRightIcon, CalendarDaysIcon, CalendarIcon, ClockIcon, SendIcon, XIcon, Trash2Icon } from "lucide-react";
...
const Schedular = () => {
  ...
  const [medilaFile, setMedilaFile] = useState<File | null>(null);
  ...
  const scheduled = posts.filter((p) => p.status === "scheduled" || p.status === "processing" || p.status === "failed");
  ...
  const handleSchedule = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (medilaFile) {
        formData.append("media", medilaFile);
      }
            ...
            <div>
              ...
              {medilaFile ? (
                <div className="relative rounded-xl overflow-hidden border border-slate-200 bg-slate-50">
                  {medilaFile.type.startsWith("image/") ? (
                    <img
                      src={URL.createObjectURL(medilaFile)}
                      alt="preview"
